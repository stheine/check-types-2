...
(function (require) {
  ...
  suite('require:', function () {
    ...
    test('identical with extra value returns false', function () {
      assert.isFalse(
        check.identical(
          { foo: 'bar', baz: 'qux', wibble: 'blee' },
          { foo: 'bar', baz: 'qux' }
    ...
    test('identical with different nested values returns false', function () {
      assert.isFalse(
        check.identical(
          { foo: 'bar', bar: { baz: { qux: 'wibble' } } },
          { foo: 'bar', bar: { baz: { qux: 'wibblee' } } },
        )
      );
    });
    ...
    test('assert modifier is applied to predicates', function () {
      ...
      assert.isFunction(check.assert.identical);
    ...
    test('assert modifier is applied to not', function () {
      ...
      assert.lengthOf(Object.keys(check.assert.not), 52);
    });

    test('assert modifier is applied to maybe', function () {
      assert.isObject(check.assert.maybe);
      assert.lengthOf(Object.keys(check.assert.maybe), 52);
    });

    test('assert modifier has correct number of keys', function () {
      assert.lengthOf(Object.keys(check.assert), 54);
    });
    ...
    test('assert throws errors with the correct messages', function () {
      ...
      assert.throws(function () { check.assert.string({}) }, 'assert failed: expected \n{}\n to be String');
      assert.throws(function () { check.assert.emptyString('"f\\o\/o"') }, 'assert failed: expected "\\"f\\\\o/o\\"" to be empty string');
      assert.throws(function () { check.assert.nonEmptyString({}) }, 'assert failed: expected \n{}\n to be non-empty string');
      ...
      assert.throws(function () { check.assert.object([]) }, 'assert failed: expected \n[]\n to be Object');
      assert.throws(function () { check.assert.emptyObject({ foo: 'bar' }) }, 'assert failed: expected \n{\n  "foo": "bar"\n}\n to be empty object');
      assert.throws(function () { check.assert.nonEmptyObject({}) }, 'assert failed: expected \n{}\n to be non-empty object');
      ...
      assert.throws(function () { check.assert.like({a: 5}, {b: 2}) }, 'assert failed: expected \n{\n  "a": 5\n}\n to be like \n{\n  "b": 2\n}\n');
      assert.throws(function () { check.assert.identical({a: 5}, {a: 6}) }, 'assert failed: expected \n{\n  "a": 5\n}\n to be identical to \n{\n  "a": 6\n}\n');
      assert.throws(function () { check.assert.array({}) }, 'assert failed: expected \n{}\n to be Array');
      assert.throws(function () { check.assert.emptyArray([ 'foo' ]) }, 'assert failed: expected \n[\n  "foo"\n]\n to be empty array');
      assert.throws(function () { check.assert.nonEmptyArray([]) }, 'assert failed: expected \n[]\n to be non-empty array');
      assert.throws(function () { check.assert.arrayLike({}) }, 'assert failed: expected \n{}\n to be array-like');
      assert.throws(function () { check.assert.iterable({}) }, 'assert failed: expected \n{}\n to be iterable');
      assert.throws(function () { check.assert.contains([], 'foo') }, 'assert failed: expected \n[]\n to contain "foo"');
      assert.throws(function () { check.assert.in('foo', []) }, 'assert failed: expected "foo" to be in \n[]\n');
      assert.throws(function () { check.assert.containsKey([], 0) }, 'assert failed: expected \n[]\n to contain key 0');
      assert.throws(function () { check.assert.keyIn(0, []) }, 'assert failed: expected 0 to be key in \n[]\n');
      assert.throws(function () { check.assert.hasLength([], 1) }, 'assert failed: expected \n[]\n to have length 1');
      assert.throws(function () { check.assert.date({}) }, 'assert failed: expected \n{}\n to be valid Date');
      assert.throws(function () { check.assert.function({}) }, 'assert failed: expected \n{}\n to be Function');
    ...
    test('not modifier has correct number of keys', function () {
      assert.lengthOf(Object.keys(check.not), 52);
    });
    ...
    test('maybe modifier has correct number of keys', function () {
      assert.lengthOf(Object.keys(check.maybe), 52);
    });
    ...
    test('array.of has predicates defined', function () {
      assert.lengthOf(Object.keys(check.array.of), 52);
      ...
      assert.isFunction(check.array.of.identical);
    ...
    test('arrayLike.of has predicates defined', function () {
      assert.lengthOf(Object.keys(check.arrayLike.of), 52);
    });
    ...
    test('iterable.of has predicates defined', function () {
      assert.lengthOf(Object.keys(check.iterable.of), 52);
    });
    ...
    test('object.of has predicates defined', function () {
      assert.lengthOf(Object.keys(check.object.of), 52);
    });
